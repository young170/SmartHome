...
int int_configure_gpio(struct gpio_dt_spec *sw_list, int sw_list_len) {
	...
	struct gpio_callback button_cb_data[4] = {button1_cb_data, button2_cb_data, button3_cb_data, button4_cb_data};

	for (int i = 0; i < sw_list_len; i++) {
		int err = gpio_pin_interrupt_configure_dt(&sw_list[i], GPIO_INT_EDGE_TO_ACTIVE);
		if (err != 0) {
			printk("Error %d: failed to configure interrupt on %s pin %d\n",
				err, gpio_sw0.port->name, gpio_sw0.pin);
			return err;
		}

		gpio_init_callback(&button_cb_data[i], btn_isr_arr[i], BIT(sw_list[i].pin));
    	gpio_add_callback(sw_list[i].port, &button_cb_data[i]);
	}
